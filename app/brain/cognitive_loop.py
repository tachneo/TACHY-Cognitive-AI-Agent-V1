...
def _draft_reply(message: str, band: str, decision: dict,
    ...
    capability_block = (
        ...
        "anything yourself.\n"
        "- TODY chat actions you CAN do when Papa asks: react/like a person's "
        "message ('like @username ka message'), reply to their message "
        "('reply @username: <text>'), star a message, or make a post "
        "('post: <text>'). These run through the same approve-first step. So if "
        "he asks you to like/reply/comment, say yes and do it — do NOT say you "
        "can't; just confirm and it goes through approval.\n"
        ...
        "- REMINDERS: you may claim a reminder/alarm is SET only when this "
        "prompt contains a [PROSPECTIVE MEMORY: reminder row ...] confirmation. "
        "Without that line, never say 'yaad rakh liya', 'reminder set', 'main "
        "yaad dila dungi' as a done fact — a promise without a persisted row "
        "gets forgotten (that is how the 3 PM reminder was lost). Instead "
        "confirm the time back and say you're noting it down, or ask them to "
    ...
    if emotion and emotion.get("enabled") and emotion.get("top_emotions"):
        ...
        emotion_block = (
            ...
            "actually reflect that. If asked how you feel, translate the listed "
            "signals into normal human Hinglish/Hindi words. Do NOT mention "
            "numeric intensities, action_bias, telemetry values, or phrases like "
            "'functional signal' / 'emotion engine' unless Papa explicitly asks "
            "for a technical debug view. Fake feelings are a lie; raw telemetry "
            "in casual chat feels robotic.\n"
