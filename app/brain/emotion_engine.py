...
def appraise(message: str, signals: Signals | None = None,
             *, related_person: str | None = None) -> dict:
    """Full emotional appraisal: detect → top-3 → gates → mood → snapshot.

    ``related_person`` (e.g. "Rohit Kumar") is attached to the emotional
    snapshot so emotions are attributed to the person they're about, not None.
    """
    s = get_settings()
    ...
    if peak >= s.emotion_snapshot_threshold:
        ...
        log_event("emotion_snapshot",
                  detail=f"top={top[0].name}@{peak}; flags={influence['flags']}"
                         f"; person={related_person or 'none'}",
                  risk_tier="low")
